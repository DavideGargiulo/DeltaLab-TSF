 ...
 * invio/ricezione messaggi e callback per eventi specifici.
 */
public class LobbyWebSocketClient extends WebSocketClient {
  ...
  private void setupDefaultHandlers() {
    ...
    onMessageType("welcome", msg -> {
      ...
      System.out.printf("[WS] Connesso alla lobby %s - Giocatori: %d/%d%n", msg.optString("lobbyId"),
          msg.optInt("players"), msg.optInt("maxPlayers"));
    });
    ...
    onMessageType("join_success", msg -> {
      ...
      System.out.printf("[WS] Join riuscita - Player ID: %s, Username: %s%n", this.playerId, this.username);
    });
    ...
    onMessageType("player_joined", msg -> {
      System.out.printf("[WS] Giocatore entrato: %s (ID: %s) - Totale: %d%n", msg.optString("username"),
          msg.optString("playerId"), msg.optInt("players"));
    });
    ...
    onMessageType("state", msg -> {
      System.out.printf("[WS] Stato lobby %s - Giocatori: %d/%d%n", msg.optString("lobbyId"), msg.optInt("players"),
      ...
      if (players != null) {
        ...
        for (int i = 0; i < players.length(); i++) {
          ...
          System.out.printf("  - %s (ID: %s)%n", player.optString("username"), player.optString("playerId"));
        }
      }
    });
    ...
    onMessageType("max_players", msg -> {
      System.out.printf("[WS] Numero massimo giocatori aggiornato: %d%n", msg.optInt("value"));
    });
  ...
  public void onClose(int code, String reason, boolean remote) {
    isConnected = false;
    System.out.printf("[WS] Connessione chiusa - Code: %d, Reason: %s, Remote: %b%n", code, reason, remote);
  ...
  private void reconnectAttempt(int attempt, int maxAttempts, int delaySeconds, CompletableFuture<Void> future) {
    if (attempt >= maxAttempts) {
      future.completeExceptionally(new Exception("Impossibile riconnettersi dopo " + maxAttempts + " tentativi"));
      return;
    }

    try {
      System.out.printf("[WS] Tentativo di riconnessione %d/%d...%n", attempt + 1, maxAttempts);
      reconnectBlocking();
      future.complete(null);
    } catch (Exception e) {
      System.err.printf("[WS] Riconnessione fallita: %s%n", e.getMessage());
      scheduler.schedule(() -> reconnectAttempt(attempt + 1, maxAttempts, delaySeconds, future), delaySeconds,
          TimeUnit.SECONDS);
    }
  }
