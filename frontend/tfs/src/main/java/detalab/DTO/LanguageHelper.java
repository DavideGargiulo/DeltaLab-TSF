...
public class LanguageHelper extends GeneralPageController {
  ...
  public static String getCodeFromName(String name) {
    return languages.entrySet().stream().filter(e -> e.getValue().equalsIgnoreCase(name)).map(Map.Entry::getKey)
  ...
  private static String translate(String text, String source, String target) throws IOException {
    ...
    BufferedReader br = new BufferedReader(
        new InputStreamReader((statusCode == 200) ? conn.getInputStream() : conn.getErrorStream(), "utf-8"));
